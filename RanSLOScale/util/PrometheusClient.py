...
class PrometheusClient:
    def __init__(self, config: Config):
        ...
        #self.prom_range_url = config.prom_no_range_url
    ...
    def execute_prom_current(self, prom_url, prom_sql):
        response = requests.get(prom_url,
                                params={'query': prom_sql,
                                        'start': int(round(time.time())),
                                        'end': int(round(time.time())),
                                        'step': self.step})
        return response.json()['data']['result'][0]['values'][0]
    ...
    def get_call(self):
        ...
        svcs = [svc for svc in k8s_util.get_svcs()]
